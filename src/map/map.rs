...
pub fn entity_map_rooms_passages (
    ...
    fill_rect(&mut commands, &mut map_objects, map_objects::WallBundle{..Default::default()}, &wall_rect);
    ...
    for _i in 0..=MAX_ROOMS {
        ...
        if ok {
            fill_rect (&mut commands, &mut map_objects, map_objects::FloorBundle{..Default::default()}, &room);

            if !rooms.is_empty() {
                let center = room.center();
                let previous_center = rooms[rooms.len()-1].center();
                if rng.gen_range(0..=1) == 1 {
                    fill_row(&mut commands, &mut map_objects, map_objects::FloorBundle{..Default::default()}, previous_center.x, center.x, previous_center.y);
                    fill_column(&mut commands, &mut map_objects, map_objects::FloorBundle{..Default::default()}, previous_center.y, center.y, center.x);
                } else {
                    fill_column(&mut commands, &mut map_objects, map_objects::FloorBundle{..Default::default()}, previous_center.y, center.y, previous_center.x);
                    fill_row(&mut commands, &mut map_objects, map_objects::FloorBundle{..Default::default()}, previous_center.x, center.x, center.y);
                }
            }
...
fn fill_rect ( commands: &mut Commands, map_objects: &mut Grid<Option<Entity>>, bundle: impl Bundle + Copy, rect: &Rectangle) {
  ...
  /*  
    for x in rect.pos1.x ..= rect.pos2.x {
        for y in rect.pos1.y ..= rect.pos2.y {
...
pub fn simple_entity_map(
    ...
    for _i in 0..100 {
        ...
        commands.spawn_bundle(map_objects::WallBundle{
